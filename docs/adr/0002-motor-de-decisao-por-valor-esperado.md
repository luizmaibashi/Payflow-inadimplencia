...
**Tentativa de correção testada e REVERTIDA (2026-08-04):** ao revisar o débito #12, tentamos reconstruir a fórmula do Gate 0 usando `previous_cnt_payment_mean` (prazo médio real dos contratos anteriores do cliente, 94,5% de cobertura, já agregado na Camada 1) — parecia a aplicação correta do princípio do ADR-0006 ("substituir premissa por medição onde há dado gratuito"). **Falhou: margem negativa em 77% dos casos.** Diagnóstico: o crédito atual precisa de ~20 meses (mediana) só para amortizar o principal, mas o histórico do cliente tem prazo mediano de 12 meses — são **populações de contrato diferentes** (anteriores = empréstimos pequenos de varejo; atual = substancialmente maior). **Lição: "existe dado disponível" ≠ "existe dado aplicável"** — o prazo histórico é medição real, mas de um objeto diferente do que se quer medir; usá-lo trocaria um viés conhecido e declarado por um erro maior e silencioso. A função `margem_via_prazo_historico_cliente()` fica em `app/motor_decisao.py` com testes, marcada como não usada em produção, como registro do experimento negativo.
