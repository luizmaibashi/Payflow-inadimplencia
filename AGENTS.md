...
- **(a) CONFIRMADA COM DADOS, CAUSA RAIZ IDENTIFICADA — o agente nunca teve acesso ao sinal que mais importa neste dataset.** Extraí os campos brutos (`utilizacao`, `pior_atraso_dias`, `deficit_medio_pct`, `n_em_atraso_hoje`) dos 564 casos e cruzei com `TARGET` real: correlação essencialmente zero em todos (`utilizacao` -0,073, `pior_atraso_dias` -0,049, `deficit_medio_pct` +0,001, `n_em_atraso_hoje` +0,029). "Sinal grave" presente (que pelo ADR-0011 justificaria `NEGAR`) tem taxa de default **menor** (30,8%, n=78) que sem sinal (33,7%, n=486) — o oposto do esperado se fosse preditivo.
  ...
  **Achado que sobra, mais importante que a causa raiz original**: dentro da zona cinzenta, **nenhum sinal disponível — nem os do agente, nem `EXT_SOURCE` do modelo campeão — separa risco de forma detectável**. Isso não é mais "o agente tem a ferramenta errada" — é evidência de que a incerteza nessa fatia específica da carteira pode ser **estrutural e genuína**, não um problema de informação faltando que dê pra resolver dando mais dado a quem decide (agente ou humano). Isso muda o que vale tentar a seguir: em vez de "dar mais sinal ao agente", a pergunta passa a ser se a zona cinzenta, definida do jeito que está, é sequer uma fatia onde decisão melhor que o acaso é possível com os dados do Home Credit.
