...
33. 🔴 **Juiz trata dado ausente como ausência de sinal, não como incerteza — REFUTADO EMPIRICAMENTE, prompt não basta** (achado 2026-08-10, fix tentado 2026-08-10, refutado no mesmo dia). 9 casos (`117727`, `143212`, `151515`, `173954`, `205320`, `222453`, `235776`, `264603`, `417173`) têm a mesma assinatura — `utilizacao`, `pior_atraso_dias`, `deficit_medio_pct` vêm `None`/indisponíveis, e o juiz conclui "nenhum sinal grave, nenhum agravante" → `FALHA`. **Tentativa de correção:** `_prompt_sistema_juiz` ganhou instrução explícita — dado indisponível não conta como sinal favorável nem como ausência de risco. Teste de regressão `test_prompt_sistema_nao_trata_dado_ausente_como_ausencia_de_risco` passa (verifica só que o texto está no prompt, não o comportamento do modelo). **Rerrodado contra API real no mesmo dia (`calibrar_juiz.py --rejulgar`, n=55): a instrução foi IGNORADA pelo `llama-3.3-70b-versatile`.** 11 dos 12 casos de discordância no novo lote são o mesmo padrão, com a evidência do juiz usando quase a mesma frase de antes ("nenhum sinal grave ou agravante presente, pois... nao disponivel") — incluindo 2 casos novos (`282047`, `285827`) que replicam o defeito. TPR caiu de 100% para 87,5% no processo (1 falso negativo novo, mas esse é ruído de ground truth já documentado, não bug do juiz). **Diagnóstico:** instrução em linguagem natural não é suficiente pra esse padrão — diferente do #32, que respondeu bem ao mesmo tipo de correção. Hipóteses pra próxima tentativa, não testadas ainda: (a) mover a instrução pra mais perto do formato de resposta esperado, (b) dar um exemplo completo de caso com dado ausente no próprio prompt (few-shot), (c) abandonar prompt e tratar como validação determinística pós-resposta (mesma família dos débitos #23/#26 — checar campos `None` no dado bruto e nos "sinais ausentes" citados na evidência do juiz, sinalizar/sobrescrever se o padrão bater, sem depender do LLM obedecer instrução).
