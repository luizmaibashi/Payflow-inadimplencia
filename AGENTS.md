...
12. **Margem/LGD do motor de decisão são proxies, não a métrica validada no Gate 0 — e o proxy de margem tem viés estrutural, não só imprecisão.** `application_train.csv` não tem `CNT_PAYMENT` (a fórmula `m = anuidade×prazo/crédito` do Gate 0 exige prazo, que não existe para uma aplicação ainda não decidida). `app/motor_decisao.py` usa `AMT_ANNUITY/AMT_CREDIT` como proxy de margem — mas essa razão **confunde prazo com margem**: dois contratos com a mesma margem total real e prazos diferentes têm razões anuidade/crédito bem diferentes (prazo curto → razão mais alta; prazo longo → razão mais baixa), mesmo sem diferença real de rentabilidade. **Diferente do Gate 0, este proxy nunca foi validado contra um sinal independente** (lá, `m_i` batia com `NAME_YIELD_GROUP`; aqui não há campo equivalente em `application_train` para cruzar). Consequência: a dispersão de `p*` usada no backtest do motor pode estar parcialmente inflada por variação de **prazo**, não só de margem/LGD genuínas — enfraquece qualquer conclusão sobre "decidir por observação agrega valor" além do que a fragilidade do threshold (débito #13) já explica sozinha. LGD usa `NAME_CONTRACT_TYPE` (Cash/Revolving — categorias diferentes das de `previous_application`). Achado de auto-crítica, 2026-08-04 (revisitado a pedido do Luiz). **Correção tentada e revertida:** usar `previous_cnt_payment_mean` (prazo histórico real do cliente) para reconstruir a fórmula do Gate 0 produziu margem negativa em 77% dos casos — o crédito atual precisa de ~20 meses para amortizar, o histórico do cliente tem mediana de 12 (populações de contrato diferentes). Lição: *"existe dado disponível" ≠ "existe dado aplicável"*. Função mantida em `app/motor_decisao.py` marcada como não-produção, com testes, como registro do experimento negativo.
13. **Backtest do motor (`reports/motor_decisao_backtest.md`) não isola desenho de motor vs. calibração do baseline — e agora tem um segundo motivo, não só um.** O delta de ~R$21 mil/caso a favor do motor de EV é majoritariamente efeito de os thresholds legados (0.40/0.65) estarem calibrados contra a escala de `p̂` de um modelo diferente (não calibrado) — contra `p̂` real, só 1% dos casos ultrapassam 0,40. Faltam **dois** braços de correção, não um: (a) um corte único recalibrado nesta mesma escala (isola calibração de desenho), e (b) revalidar se a dispersão de `p*` por observação é genuína ou artefato do proxy de margem confundir prazo com margem (débito #12). Sem os dois, o que este backtest prova é só que threshold fixo é frágil a mudança de calibração do modelo — não que decidir por observação tenha valor incremental de verdade.
14. 🔴 **DEFEITO CONFIRMADO (auditoria 2026-08-04): o proxy de margem é NEGATIVAMENTE correlacionado com a margem verdadeira (Spearman ρ = −0,40).** Medido na população onde as duas fórmulas são calculáveis (`previous_application`, n=939.001): margem verdadeira mediana 23,4% × proxy 10,9%. Não é só subestimação de nível — é **inversão de ordenação**. Motivo algébrico: `m_true = m_proxy × prazo − 1`, e `m_proxy = anuidade/crédito ≈ 1/prazo`; logo o proxy mede essencialmente "quão curto é o contrato", e contratos curtos têm **menos** juros totais. **Consequência:** o motor atribui `p*` mais baixo justamente aos contratos mais rentáveis — o ajuste por observação está sistematicamente na direção errada, e um `p*` global seria melhor que o atual por observação. **Bloqueante para qualquer alegação de valor do motor.**
15. 🔴 **O veredito "APROVADO" do Gate 0 não transfere para o motor como implementado.** O Gate 0 mediu IQR de `p*` = 18,0pp usando a fórmula verdadeira em `previous_application`; o motor real, em `application_train` com o proxy, tem IQR de **3,4pp** — contra um critério de reprovação de 3,0pp, ou seja, na fronteira. O gate validou uma população e uma fórmula que o motor não usa. Reexecutar o Gate 0 sobre a métrica efetivamente usada antes de sustentar a decisão por observação.
16. **Banda de indiferença do motor (±3pp) é largura fixa simplificada**, não derivada da incerteza real da estimativa de PD por caso (ADR-0002 §2.6 previa isso, ainda não implementado).
