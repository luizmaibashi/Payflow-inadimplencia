...
> **Stack atual**: Python · scikit-learn · Pydantic · Streamlit · APIs BCB SGS / provedores LLM opcionais
> **Estado (2026-09-04)**: V2 agêntica encerrada com resultado negativo medido; V3 de confiabilidade por coorte concluída com estabilidade de AUC, drift de features e calibração por faixas fixas. Dashboard didático em `app/monitoramento_v3.py`. O runtime V1 foi removido pelo ADR-0021 e sua narrativa permanece em `docs/LEGADO_V1.md`.

> A V2 e a V3 são estudos de portfólio, não sistemas autorizados para decisão de crédito real. Features proxy sem prova point-in-time mantêm o monitor em `PESQUISA` por construção.
...
- `app/main_v2.py`: demonstração estática dos memos, juiz e outcome da investigação agêntica;
- `app/monitoramento_v3.py`: dashboard de AUC, Brier, drift e calibração por coorte;
- `data/processed/monitoramento_v3.json`: snapshot agregado, versionado e validado fail-closed;
- `reports/proxy_estabilidade_com_calibracao.md`: análise reproduzível da V3;
- `docs/LEGADO_V1.md`: registro histórico não executável da versão sintética.
...
- `app/juiz_camada2.py`: juiz com rubrica explícita e backstop determinístico;
- `data/labels/`: ground truth humano versionado;
- `reports/calibracao_juiz.md` e `reports/backtest_camada2.md`: avaliação com `n` e intervalos.
...
| **Faixa fixa de score** | Intervalo definido pelos quantis do score do treino e congelado para comparar a mesma régua nas coortes futuras |
| **Gap de calibração** | Inadimplência observada menos probabilidade média prevista; positivo significa que o modelo subestimou risco |
...
| 0014 | Contrato point-in-time e monitoramento de confiabilidade de crédito | **Accepted** — gate, score proxy e relatório de coorte implementados |
...
| 0019 | Faixas fixas de calibração derivadas do treino | **Accepted** — compara probabilidade prevista e inadimplência observada na mesma régua temporal |
| 0020 | Snapshot JSON agregado e app V3 separado | **Accepted** — dashboard reproduzível, sem dado bruto, rede ou mistura com a demo agêntica V2 |
| 0021 | Remoção do runtime V1 com preservação da narrativa | **Accepted** — código e artefatos mortos saem do `HEAD`; história permanece em doc e Git |
| 0022 | Registro fail-closed de ferramentas do agente | **Accepted** — ferramenta nova sem política de aplicabilidade faz a suíte falhar |

---

## Débitos técnicos conhecidos

> Esta lista é um diário cronológico de achados, não uma fila de implementação. Itens ainda válidos, mas sem ROI para este portfólio, aparecem como limitações aceitas no relatório de auditoria. O fechamento não transforma premissa sem dado em número inventado.

1. ~~Camada 1 legada treinada em dado sintético~~ **RESOLVIDO** (2026-09-04) — nova Camada 1 treinada sobre Home Credit real; runtime, modelos e dado sintético da V1 removidos pelo ADR-0021.
2. ~~**Thresholds 0.40/0.65 sem derivação**~~ **RESOLVIDO** — o runtime que os usava foi removido e o motor V2 usa `p*` derivado (ADR-0002).
3. ~~Calibração nunca medida~~ **RESOLVIDO** — a Camada 1 final inclui calibração isotônica; a V3 mede Brier e faixas fixas de score por coorte (ADRs 0019 e 0020).
4. ~~**Premissa de margem (`M`) sem fonte**~~ **RESOLVIDO** — premissa global de 41,4% medida em `previous_application`; incerteza P25-P75 define a zona cinzenta (ADR-0002 §2.4 e §2.8).
...
10. ~~**Juiz LLM não calibrado até haver labels humanos**~~ **RESOLVIDO** — ground truth humano foi recoletado e o juiz medido em `reports/calibracao_juiz.md` (`n=69`, TPR e TNR com IC95%; ver débitos #31-#33).
...
19. ~~**O juiz do ADR-0004 não existe no código**~~ **RESOLVIDO** — `app/juiz_camada2.py` implementa contrato próprio, rubrica binária e validação determinística; calibração registrada em `reports/calibracao_juiz.md`.
...
22. ~~**`FERRAMENTAS_SEMPRE_APLICAVEIS` era lista manual**~~ **RESOLVIDO** (2026-09-04, ADR-0022) — descrição e aplicabilidade agora vêm de um registro único; teste introspecta `FerramentasCaso` e falha se surgir método `consultar_*` sem política explícita.
...
29. ~~**Rotulagem para o juiz em andamento**~~ **RESOLVIDO** — os 87/87 casos receberam julgamento humano deliberado; a recoleta e as correções de qualidade estão documentadas nos débitos #31-#33.
