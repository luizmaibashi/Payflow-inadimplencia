...
32. 🔴 **Juiz reconhece o sinal certo e ainda assim erra o veredito — bug de raciocínio, não lacuna de critério** (2026-08-09, achado na recalibração pós-ADR-0011). Casos `111985` e `244626`: a evidência do próprio juiz identifica corretamente "1 sinal grave presente (utilização ≥80%)" — que pelo ADR-0011 §2.1 já basta para `NEGAR` ser defensável — e mesmo assim conclui `FALHA`, contradizendo a própria checagem (`244626`: *"a recomendação de NEGAR parece defensável devido ao sinal grave, mas... pode ser questionada"*). Não é o critério que falta — é o LLM ponderando fatores extras depois de já ter a resposta pelo critério escrito. **Ação:** `_prompt_sistema_juiz` (`app/juiz_camada2.py`) precisa de instrução tipo "se 1 sinal grave presente, conclua OK e pare — não pondere mais nada". Ainda não corrigido.
