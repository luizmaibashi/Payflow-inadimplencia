...
- **(a) CONFIRMADA COM DADOS — o critério do ADR-0011 não é preditivo de `TARGET` nesta população.** Extraí os campos brutos (`utilizacao`, `pior_atraso_dias`, `deficit_medio_pct`, `n_em_atraso_hoje`) dos 564 casos e cruzei com `TARGET` real. Correlação com `TARGET`: todas essencialmente zero (`utilizacao` -0,073, `pior_atraso_dias` -0,049, `deficit_medio_pct` +0,001, `n_em_atraso_hoje` +0,029). Mais direto ainda: clientes com "sinal grave" presente (que pelo ADR-0011 justificaria `NEGAR`) têm taxa de default **menor** (30,8%, n=78) que os sem sinal (33,7%, n=486) — o oposto do esperado se o sinal fosse preditivo. **Isso explica o resultado central**: o agente pode estar aplicando o critério corretamente (é o que os débitos #32/#33 garantiram) e mesmo assim não separar risco, porque o critério em si mede defensabilidade de raciocínio, não probabilidade real de default. Não verificado ainda: se a Camada 1 (AUC 0,776 real) usa essas mesmas variáveis de bureau com peso relevante — ficou bloqueado por `models/camada1_home_credit_v1.pkl` ser um `HistGradientBoostingClassifier` congelado (`FrozenEstimator`) sem `feature_importances_` acessível sem rodar importância por permutação contra o dataset bruto (2,6GB, só nesta máquina).
