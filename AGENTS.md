...
26. 🟡 **Groundedness mecânica valida "a ferramenta foi chamada", não "o número bate"** (2026-08-06, achado na leitura manual dos 5 casos-piloto). `validar_groundedness()` (`app/agente_underwriting.py`) confere se `fonte_tool` resolve contra uma chamada real — pega alucinação de nome de ferramenta (achado real: caso `292411`, `fonte_tool` veio como `"consultar_bureau, consultar_pagamentos"`, dois nomes concatenados, nem um dos dois existe). Não confere se o *valor numérico* citado no fato realmente aparece no retorno daquela ferramenta — essa checagem exige leitura humana ou juiz LLM (débito #19), é o que a rubrica de groundedness do ADR-0004 pressupõe mas o código só cobre a metade mecânica.
