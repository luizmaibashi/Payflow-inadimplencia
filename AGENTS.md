...
34. 🟡 **Backtest da Camada 2 contra `TARGET` real — implementado, aguardando `n` com poder estatístico** (2026-08-11). `scripts/backtest_camada2.py` era esqueleto puro (5 funções com `NotImplementedError`) desde 2026-08-08 — a medição que o ADR-0004 §2.1 chama de "a contribuição do projeto" nunca tinha sido escrita, só uma conta ad-hoc no docstring. **Implementado:** `carregar_memos_com_desfecho`, `juntar_com_target` (merge com `zona_cinzenta.parquet`), `taxa_default_por_recomendacao` (Wilson, reaproveita `intervalo_wilson` de `calibrar_juiz.py`), `bootstrap_separacao` (2000 reamostragens, ADR-0004 §2.5). **Validado contra os 86 casos existentes** (grátis, antes de gastar API): reproduziu quase exatamente a medição ad-hoc do docstring (APROVAR 30,6%, NEGAR 37,8%, separação +7,2pp IC95% [-12,4%; +28,3%]) — confirma que a implementação está correta. **`comparar_com_motor` documentada como não implementável com os dados atuais**, não só deixada em branco: `zona_cinzenta.parquet` tem `decisao_motor` constante (`"ZONA_CINZENTA"` em 100% das linhas) porque esse arquivo já É o recorte onde o motor se absteve — não existe uma decisão real do motor pra comparar dentro desse universo. Uma comparação alternativa (proxy via `p_hat` vs. ponto médio de `p_estrela_inf/sup`) é decisão de design nova, não bloqueia o resultado central. **Bloqueante restante:** `n=86` é pequeno demais pra separação de 10pp (exige ~722, poder 0,80). Rodar `python scripts/piloto_camada2.py --n 722 --seed 42 --modelo gemini-2.5-flash` (~US$7, medido em US$0,0096/caso no smoke test de 5 — melhor que a estimativa de US$0,0109) — sessão dedicada, provavelmente várias horas sem paralelismo. **Cuidado:** esse comando sobrescreve `data/processed/piloto_camada2_memos.jsonl`, que é o ground truth versionado do débito #31 — copiar antes de rodar.
