...
- **(a) CONFIRMADA COM DADOS, CAUSA RAIZ IDENTIFICADA — o agente nunca teve acesso ao sinal que mais importa neste dataset.** Extraí os campos brutos (`utilizacao`, `pior_atraso_dias`, `deficit_medio_pct`, `n_em_atraso_hoje`) dos 564 casos e cruzei com `TARGET` real: correlação essencialmente zero em todos (`utilizacao` -0,073, `pior_atraso_dias` -0,049, `deficit_medio_pct` +0,001, `n_em_atraso_hoje` +0,029). "Sinal grave" presente (que pelo ADR-0011 justificaria `NEGAR`) tem taxa de default **menor** (30,8%, n=78) que sem sinal (33,7%, n=486) — o oposto do esperado se fosse preditivo.
  ...
  **Tensão de design a resolver antes de agir**: expor `EXT_SOURCE_1/2/3` ao agente resolveria o problema de poder preditivo, mas esbarra no espírito do ADR-0003 §2.1 (cegueira ao score) — não são literalmente `p_default`, mas são a entrada dominante (94% da importância combinada das top 3) que alimenta o modelo que calcula `p_default`. Dar ao agente essas 3 variáveis pode reduzir a "Camada 2" a parafrasear um proxy quase tão forte quanto o score, na prática reintroduzindo a ancoragem que o ADR-0003 existe pra evitar — mesmo passando no teste literal de "nem p_default nem derivado aparece no contexto". Decisão de arquitetura pendente, não resolvida aqui: registrar como novo ADR quando for decidir.
