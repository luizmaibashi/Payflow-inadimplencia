...
31. 🔴 **63 dos 77 labels `OK` de Task Completion não são julgamentos — são conferências de número na rubrica errada** (2026-08-08, achado na primeira calibração do juiz). Classificando a linguagem das evidências: dos 77 `OK`, **63 usam linguagem de groundedness** ("tudo bate", "os valores existem", "todas as informações batem com os dados brutos"), **8 estão vazios**, e apenas **8 julgam de fato a recomendação** ("é defensável porém...", "já tem 92% do crédito comprometido"). Dos 10 `FALHA`, **10 de 10** julgam a recomendação. **Causa raiz é a ferramenta, não o revisor:** `revisor_camada2.html` empilhava as 4 rubricas com quatro `textarea` visualmente idênticos — no caso 40 de 87, "tudo bate" é escrito uma vez e escorre para os quatro campos. O JSON sai completo e válido, então **nada acusa**. Mesma família dos débitos #28 e #30. **Consequência medida:** o TNR de 89,6% do `reports/calibracao_juiz.md` foi calculado contra ~60 não-julgamentos e **não vale**; o TPR sobrevive porque a classe `FALHA` é composta só de julgamentos reais. **Correções aplicadas:** (a) ADR-0011 escreve o critério de Task Completion como regra verificável, (b) a tela de rotulagem passou a fazer **uma pergunta por caso** em vez de quatro campos livres (`scripts/gerar_revisor_verificacao.py`), (c) as rubricas mecânicas (groundedness, trajetória, cegueira) deixam de pedir julgamento humano — vêm dos validadores. **Recoleta parcial (2026-08-09):** 23 dos 87 casos (11 novos + 8 label frágil + 4 OK suspeito por proxy de peso) rejulgados sob ADR-0011 — 9 FALHA + 14 OK, evidências agora citam fato específico. `240344` corrigido de `OK`→`FALHA` no mesmo dia (erro de clique confirmado pelo revisor — o caso é o exemplo-âncora do ADR-0011 §2.2.2, permanece `FALHA`). **Escopo declarado, não bug:** os outros 64 `OK` de 2026-08-08 (a maioria dos 63 contaminados originais) **não foram recoletados** — decisão consciente de não rotular os 77 de novo, ver conversa 2026-08-09. `data/labels/task_completion_labels.json` tem 98 registros agora (87 + 11 casos novos que não existiam no lote anterior). **Consequência para o TNR:** ainda não vale — a maior parte da classe `OK` segue sendo conferência de número, não julgamento.
32. ~~🔴 **Juiz reconhece o sinal certo e ainda assim erra o veredito — bug de raciocínio, não lacuna de critério**~~ **PROMPT CORRIGIDO, VALIDAÇÃO EMPÍRICA PENDENTE** (2026-08-09, achado na recalibração pós-ADR-0011). Casos `111985` e `244626`: a evidência do próprio juiz identificava corretamente "1 sinal grave presente (utilização ≥80%)" — que pelo ADR-0011 §2.1 já basta para `NEGAR` ser defensável — e mesmo assim concluía `FALHA`, contradizendo a própria checagem (`244626`: *"a recomendação de NEGAR parece defensável devido ao sinal grave, mas... pode ser questionada"*). Não era o critério que faltava — era o LLM ponderando fatores extras depois de já ter a resposta pelo critério escrito. **Correção aplicada:** `_prompt_sistema_juiz` (`app/juiz_camada2.py`) ganhou instrução explícita — "sinal/agravante confirmado → veredito decidido, não existe segunda camada de análise". Teste de regressão `test_prompt_sistema_proibe_segunda_camada_de_analise`, 160/160 passando. **Em aberto:** só muda o texto do prompt — não foi testado contra API real (LLM pode ignorar a instrução na prática). Validação fica pro próximo `calibrar_juiz.py --rejulgar`, quando a cota Groq resetar.
33. ~~🔴 **Juiz trata dado ausente como ausência de sinal, não como incerteza — bug distinto do #32**~~ **PROMPT CORRIGIDO, VALIDAÇÃO EMPÍRICA PENDENTE** (2026-08-10, achado na releitura completa dos 13 casos de discordância de `reports/calibracao_juiz.md`, n=52). Separando os 13 por padrão: **9 casos** (`117727`, `143212`, `151515`, `173954`, `205320`, `222453`, `235776`, `264603`, `417173`) têm a mesma assinatura — `utilizacao`, `pior_atraso_dias`, `deficit_medio_pct` vêm `None`/indisponíveis, e o juiz conclui "nenhum sinal grave, nenhum agravante" → `FALHA`, porque a recomendação `NEGAR` "não se justifica" sem sinal. Mas dado ausente não é dado favorável — pode ser exatamente o motivo da cautela (histórico insuficiente para avaliar). Isso é **diferente** do #32: lá o juiz via o sinal e ponderava demais; aqui o juiz não vê sinal nenhum porque trata a lacuna como zero. O fix do #32 (parar de ponderar após achar sinal) não toca esse padrão — não há sinal achado para parar de ponderar. Dos 13 casos originais, só `244626` continua sob o guarda-chuva do #32; `111985` é uma variante (sinal grave achado, mas o juiz inventa uma condição extra não prevista no critério — "memo não menciona pior_atraso <30d e n_em_atraso=0" — para ainda assim marcar `FALHA`); `240344` já foi corrigido na recoleta ADR-0011 e não conta mais. **Correção aplicada:** `_prompt_sistema_juiz` (`app/juiz_camada2.py`) ganhou instrução explícita — dado indisponível (`None`/"não disponível") não conta como sinal favorável nem como ausência de risco, é incerteza, e por si só nunca torna `NEGAR` indefensável; avaliar só os sinais/agravantes de fato presentes. Teste de regressão `test_prompt_sistema_nao_trata_dado_ausente_como_ausencia_de_risco`, 17/17 passando em `tests/test_juiz_camada2.py`. **Em aberto:** mesma ressalva do #32 — só muda o texto do prompt, não foi testado contra API real. **Consequência para a próxima calibração:** rerrodar `calibrar_juiz.py --rejulgar` agora carrega os fixes de #32 e #33 juntos — se o TNR não subir de forma expressiva mesmo assim, é sinal de um terceiro padrão ainda não mapeado, não de que os fixes falharam.
