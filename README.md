...
O passo mais importante não foi medir isso — foi investigar por quê, em vez de parar em "não funcionou". Voltando aos dados brutos dos 564 casos: os sinais que o critério do agente usa pra justificar `NEGAR` (utilização de crédito, pior atraso, déficit de pagamento) têm correlação essencialmente **zero** com o `TARGET` real — e clientes com "sinal grave" presente até defaultam **menos**, não mais, que os sem sinal (30,8% vs. 33,7%). Isso muda o diagnóstico: não é que o agente aplica mal um critério bom — é que o critério em si mede se o raciocínio é *defensável*, não se é *preditivo*. As duas coisas parecem a mesma coisa até serem medidas separadamente, e não são.

A conclusão não é "agentes LLM não decidem crédito bem" — é mais específica e mais útil: com este critério, este dataset e este agente, o instrumento de decisão está otimizado pra parecer coerente, não pra prever o desfecho real. Isso aponta pra um próximo passo concreto (redesenhar o critério em cima de sinais que de fato correlacionam com `TARGET`, não sinais que só soam razoáveis), em vez de deixar a pergunta em aberto. Detalhamento completo da investigação, incluindo os números por variável, está no débito #34 do [`AGENTS.md`](AGENTS.md).
